...
```{r}
bulk.data.merged.QC.tpm %>%filter(tpm > 4)  %>%
  mutate(log_tpm = log10(tpm+1))%>%
  select(gene_id,sample,log_tpm) %>%
  pivot_wider(values_from = "log_tpm", names_from = "sample" ,values_fill = 0 ) %>%
  column_to_rownames("gene_id")-> normExpression.parts


normExpression.parts
...
as.data.frame(mir.pca$x) %>% rownames_to_column(var = "sampleName") %>%
ggplot( aes(x = PC1, y = PC2, label = sampleName)) + geom_point()+ geom_text()
